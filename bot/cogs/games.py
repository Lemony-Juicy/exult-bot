...
class Games(commands.Cog):
    ...
    @commands.command(description="Play Connect 4 with a friend.")
    ...
    @commands.command(description="Test how fast your reaction time is.")
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def reaction_test(self, ctx):
        message = await ctx.send("Press the reaction when the green emoji appears. 🔴")
        message.add_reaction('👍')
        time = random.choice(range(1, 10))
        await asyncio.sleep(time)
        t = time.time()
        await message.edit("Press the reaction when the green emoji appears. 🟢")
        def check(reaction, user):
            return (
        ...
        try:
            ...
            return await ctx.send("You took too long to react.")
        t2 = time.time()
        t = (t2 - t) * 1000
        
        await ctx.send(f"You took {t} seconds to react.")
