...
class Warning(commands.Cog):
    ...
    async def warn(self, ctx, member: discord.Member, reason: str):
        """Warn a member"""
        await self.db.add_warn(ctx.guild.id, member.id, ctx.author.id, reason)
        embed = discord.Embed(description=f"**Reason:** {reason}\n**Warned at:** {kimetsu.parsedate.Parsedate().parsedate()}", colour=self.bot.red)
    ...
    async def warns(self, ctx, member: discord.Member=None):
        ...
        member_warns = await self.db.get_warns_for_member(ctx.guild.id, member.id)
    ...
    @commands.command()
    @commands.has_permissions(manage_messages=True)
    async def delwarn(self, ctx, warning_id):
        """Delete a warn via its ID"""
        x = await self.db.get_warn(int(warning_id))
        if not x:
        ...
        await self.db.remove(int(warning_id))
        user = self.bot.get_user(x[2])
        moderator = self.bot.get_user(x[3])
        reason = x[4]
    ...
    async def delwarns(self, ctx, member: Union[discord.Member, str]):
        if isinstance(member, discord.Member):
            x = await self.db.get_warns_for_member(ctx.guild.id, member.id)
            if type(x) == list:
                total_warns = len(x)
                await self.db.clear_member(ctx.guild.id, member.id)
        elif isinstance(member, str):
            if member.lower() == "all":
                x = await self.db.clear_guild(ctx.guild.id)
            else:
                return
        if type(x) == int:
            if x <= 0:
    ...
    async def editwarn(self, ctx, warning_id: int, *, new_reason: str):
        x = await self.db.get_warn(warning_id)
        if x is None:
        ...
        member = self.bot.get_user(x[2])
        moderator = self.bot.get_user(x[3])
        old_reason = x[4]
        await self.db.update(warning_id, new_reason)
